        ...
        for line in f:
            if line.strip() and not line.startswith("#"):  # Ignorar líneas vacías o comentarios
                ...
                if punto_fijo:
                    ...
                    data.append((P_fx, Vt_fx, result_fx))
                else:
                    data.append((P_raw, Vt, result))
    return data

@pytest.mark.parametrize("P_fx, Vt_fx, expected_fx",
    [pytest.param(P_raw, Vt, expected, id=f"P={P_raw}, Vt={Vt}")
     for P_raw, Vt, expected in leer_datos()])
def test_calc_density(P_fx, Vt_fx, expected_fx):
    if punto_fijo:
    ...
    assert relativo < tolerancia, \
        f"Fallo: P={P_fx}, Vt={Vt_fx}, Esperado={expected_float}, Obtenido={result_float}, Error relativo={relativo:.4f}"
