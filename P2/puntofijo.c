...
void calc_kinetic_energy_fx(uint32_t dato_A, uint32_t dato_W,uint32_t *resultado) {
  ...
  printf("Ek: %f,%f (guarda %.2f bits)\n", Ek, (float)Ek_fx/(1<<16), 32-log2(Ek_fx));
...
int main() {
  ...
  for (char contador=0;contador<4;contador++){
    ...
    for (int contador=0;contador<4;contador++){
        ...
        printf("Ejercicio 3(%f %f): %.6f J\n\n", tabla_ener[contador][0],tabla_ener[contador][1], (float)salida/(1<<16));
    }
