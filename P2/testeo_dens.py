...
if punto_fijo:
    ...
    lib.calc_density_fx.argtypes = [ctypes.c_uint32, ctypes.c_uint32, ctypes.POINTER(ctypes.c_uint32), ctypes.POINTER(ctypes.c_float)]
...
def probar_calc_density():
    ...
    for P_fx, Vt_fx, expected_fx, expected_raw in datos:
        if punto_fijo:
            ...
            res_fl = ctypes.c_float()

            lib.calc_density_fx(dato_P, dato_Vt, ctypes.byref(resultado), ctypes.byref(res_fl))
        ...
        if relativo < tolerancia:
            print(f"Éxito: P={P_fx}, Vt={Vt_fx}, Esperado= {expected_float}, Obtenido={result_float}, Error relativo={relativo:.4f}")
        else:
            print(f"Fallo: P={P_fx}, Vt={Vt_fx}, Esperado={expected_float}, Obtenido={result_float}, Error relativo={relativo:.4f}")
