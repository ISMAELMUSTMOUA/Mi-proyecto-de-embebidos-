...
@pytest.mark.parametrize("A_fx, W_fx, expected_fx",
    [pytest.param(A_raw, W_raw, expected, id=f"A={A_raw}, W={W_raw}")
     for A_raw, W_raw, expected in leer_datos()])
def test_calc_kinetic_energy(A_fx, W_fx, expected_fx):
    if punto_fijo:
    ...
    assert relativo < tolerancia, \
        f"Fallo: A={A_fx}, W={W_fx}, Esperado={expected_float}, Obtenido={result_float}, Error relativo={relativo:.4f}"
