...
void calc_density_fx(uint32_t dato_P, uint32_t datoVt,uint32_t *resultado,float *res_float) {
  ...
  //punto flotante 
    ...
    float P_float = dato_P/(float)(1<<tams_des[0]); // Convertir a flotante
    float P = (P_float - C0)/C1;

    float Vt_float = datoVt/(float)(1<<tams_des[1]); // Convertir a flotante
    ...
    *res_float = rho;
    ...
    uint32_t rho_fx =  FX_DIV(P_fx, B_fx, tams_des[2]) ;

    // --- Resultados y comparación ---
    printf("Escalado P: %d     Escalado Vt: %d\n",tams_des[0],tams_des[1]);
    ...
    printf("rho:   %f,%f (guarda %.2f bits)\n",rho,(float)rho_fx/(1<<tams_des[2]),32-log2(rho_fx));
    *resultado = rho_fx;
}
...
void calc_kinetic_energy_fx(uint32_t dato_A, uint32_t dato_W,uint32_t *resultado, float *res_float) {
  ...
  // A_g = (dato_A * 4) / 1024
  uint32_t A_g_fx = FX_DIV(FX_MUL(dato_A, CONST_4_fx, 8), CONST_1024_fx, 8);
  // A = A_g * 9.81
  ...
  printf("V^2:  %f,%f (guarda %.2f bits)\n", v2, (float)v2_fx/(1<<8), 32-log2(v2_fx));
  printf("Ek: %f,%f (guarda %.2f bits)\n", Ek, (float)Ek_fx/(1<<tams_ener[2]), 32-log2(Ek_fx));
